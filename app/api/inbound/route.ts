...
export async function POST(request: NextRequest) {
  ...
  if (resultWebhook && resultSecret) {
    ...
    const callback = {
      ...
      decisao: body.decisao || "aprovar",
      decisao_em: body.decisao_em || new Date().toISOString(),
      wordpress_post_id: wordpress?.id || null,
      wordpress_url: wordpress?.link || null,
      status: callbackStatus,
      pwa_status: callbackStatus,
      processado_ia_em: body.processado_ia_em || new Date().toISOString(),
      title: row.title,
      excerpt: row.excerpt,
      content: row.content,
