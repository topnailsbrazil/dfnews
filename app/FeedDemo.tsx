...
export default function FeedDemo({ stories }: { stories: Array<{ category: string; title: string; excerpt: string; tone: string; image_url?: string | null; source_name?: string | null }> }) {
    ...
    <section className="feed" aria-label="Feed de notícias">
      {visible.length ? visible.map((story, index) => <div key={story.title}>
        <article className={`story ${story.tone}`} style={story.image_url ? { backgroundImage: `url(${story.image_url})` } : undefined} onClick={() => setOpenStory(story)} onTouchStart={(event) => { touchStart.current = event.touches[0]?.clientY || null; }} onTouchEnd={(event) => { const start = touchStart.current; const end = event.changedTouches[0]?.clientY || start || 0; if (start !== null && start - end > 45) setOpenStory(story); touchStart.current = null; }}>
          <div className="story-overlay" /><div className="story-content"><div className="story-meta"><span>{story.category}</span><span>{story.source_name || "DFJÁ agora"}</span></div><h1>{story.title}</h1><p>{story.excerpt}</p><div className="story-hint">‹ &nbsp; Deslize para ler &nbsp; ›</div><div className="story-footer"><span>DFJÁ</span><span>{String(index + 1).padStart(2, "0")}</span></div></div><div className="social-rail" aria-label="Menu social da notícia"><div className="story-avatar" aria-label="Ícone DFJÁ">DF<span>JÁ</span></div><button className={liked[story.title] ? "social-action is-liked" : "social-action"} type="button" aria-label="Curtir" onClick={() => setLiked((state) => ({ ...state, [story.title]: !state[story.title] }))}><span>♥</span><small>{liked[story.title] ? 1 : 0}</small></button><button className="social-action" type="button" aria-label="Compartilhar" onClick={async () => { try { await navigator.clipboard?.writeText(window.location.href); } catch {} setShared(story.title); setTimeout(() => setShared(null), 1600); }}><span>↗</span><small>{shared === story.title ? "Copiado" : "Enviar"}</small></button><button className={commenting === story.title ? "social-action is-commenting" : "social-action"} type="button" aria-label="Comentar" onClick={() => setCommenting(commenting === story.title ? null : story.title)}><span>▢</span><small>{comments[story.title]?.length || 0}</small></button></div>
  ...
  </div>{openStory && <div className="story-reader" role="dialog" aria-modal="true" onClick={() => setOpenStory(null)}><article onClick={(event) => event.stopPropagation()}><button className="reader-close" type="button" aria-label="Fechar matéria" onClick={() => setOpenStory(null)}>×</button>{openStory.image_url && <img src={openStory.image_url} alt="" />}<div className="reader-content"><div className="story-meta"><span>{openStory.category}</span><span>DFJÁ • agora</span></div><h2>{openStory.title}</h2><p className="reader-lead">{openStory.excerpt}</p><p>Esta é uma prévia da matéria reescrita pelo DFJÁ. O conteúdo final será produzido pela IA a partir de fontes selecionadas, conferido pelo editor e apresentado com linguagem própria, contexto local e informações verificadas.</p><p>Na publicação completa, o leitor encontrará os principais fatos, os dados disponíveis e a referência da fonte original. A matéria só será publicada depois da aprovação editorial pelo WhatsApp.</p><div className="reader-source">Fonte de referência: portal selecionado • Texto demonstrativo</div></div></article></div>}</div></div>;
}
