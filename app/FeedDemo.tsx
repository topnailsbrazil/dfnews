    ...
    <section className="feed" aria-label="Feed de notícias">
      {visible.length ? visible.map((story, index) => <div key={story.title}>
        <article className={`story ${story.tone}`} style={story.image_url ? { backgroundImage: `url(${story.image_url})` } : undefined} onClick={() => setOpenStory(story)} onTouchStart={(event) => { touchStart.current = event.touches[0]?.clientY || null; }} onTouchEnd={(event) => { const start = touchStart.current; const end = event.changedTouches[0]?.clientY || start || 0; if (start !== null && start - end > 45) setOpenStory(story); touchStart.current = null; }}>
          <div className="story-overlay" /><div className="story-content"><div className="story-meta"><span>{story.category}</span><span>{story.source_name || "DFJÁ agora"}</span></div><h1>{story.title}</h1><p>{story.excerpt}</p><div className="story-hint">‹ &nbsp; Deslize para ler &nbsp; ›</div><div className="story-footer"><span>DFJÁ</span><span>{String(index + 1).padStart(2, "0")}</span></div></div><div className="social-rail" aria-label="Menu social da notícia"><div className="story-avatar" aria-label="Ícone DFJÁ">DF<span>JÁ</span></div><button className={liked[story.title] ? "social-action is-liked" : "social-action"} type="button" aria-label="Curtir" onClick={(event) => { event.stopPropagation(); setLiked((state) => ({ ...state, [story.title]: !state[story.title] })); }}><Icon type="like" /><small>{liked[story.title] ? 1 : 0}</small></button><button className="social-action" type="button" aria-label="Compartilhar" onClick={async (event) => { event.stopPropagation(); try { await navigator.clipboard?.writeText(window.location.href); } catch {} setShared(story.title); setTimeout(() => setShared(null), 1600); }}><Icon type="share" /><small>{shared === story.title ? "Copiado" : "Share"}</small></button><button className={commenting === story.title ? "social-action is-commenting" : "social-action"} type="button" aria-label="Comentar" onClick={(event) => { event.stopPropagation(); setCommenting(commenting === story.title ? null : story.title); }}><Icon type="comment" /><small>{comments[story.title]?.length || 0}</small></button></div>
